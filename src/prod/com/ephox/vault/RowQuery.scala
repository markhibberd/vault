...
sealed trait RowQuery[L, A] {
  ...
  def map[B](f: A => B): RowQuery[L, B] =
    rowQuery(s => this <|- s map f)

  def flatMap[B](f: A => RowQuery[L, B]): RowQuery[L, B] =
    rowQuery(s => (this <|- s) flatMap (a => f(a) <|- s))
}

trait RowQuerys {
  def rowQuery[L, A](f: Query => RowConnect[L, A]): RowQuery[L, A] = new RowQuery[L, A] {
    def <|-(sql: Query) =
      f(sql)
  }
  ...
  implicit def RowQueryPure[L, M[_]]: Pure[({type λ[α]= RowQuery[L, α]})#λ] = new Pure[({type λ[α]= RowQuery[L, α]})#λ] {
    def pure[A](a: => A) =
      rowQuery(_ => a.η[({type λ[α]= RowConnect[L, α]})#λ])
  }
